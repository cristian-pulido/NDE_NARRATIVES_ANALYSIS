...
from nde_narratives.config import load_paths_config, load_study_config, load_translate_config
...
def test_run_translate_pipeline_writes_translated_outputs_and_detected_language(tmp_path: Path) -> None:
    study = load_study_config(FIXTURES / "study_test.toml")
    paths_config = make_paths_config(tmp_path, FIXTURES / "survey_fixture.csv", llm_block=_translate_block())
    ...
    assert translated.iloc[0][study.sections["context"].source_column] == "translated context"
    assert translated.iloc[0][study.sections["experience"].source_column] == "translated experience"
    assert translated.iloc[0][study.sections["aftereffects"].source_column] == "translated aftereffects"
...
def test_translate_command_executes_with_translate_block(tmp_path: Path) -> None:
    ...
    result = run_cli("translate", "--study-config", str(study_config), "--paths-config", str(paths_config), "--limit", "1")
...
def test_translate_tolerates_literal_newlines_in_model_json_string(tmp_path: Path) -> None:
    study = load_study_config(FIXTURES / "study_test.toml")
    paths_config = make_paths_config(tmp_path, FIXTURES / "survey_fixture.csv", llm_block=_translate_block())
    paths = load_paths_config(paths_config)
    translate = load_translate_config(paths_config)
    ...
    class NewlineLiteralProvider:
        def generate_structured(self, request) -> LLMProviderResponse:
            raw = '{\n  "translation": "line 1\nline 2",\n  "source_language": "nl"\n}'
            return LLMProviderResponse(provider="fake", model=request.model, raw_text=raw)
