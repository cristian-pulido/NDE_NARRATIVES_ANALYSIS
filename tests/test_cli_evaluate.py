...
def _set_annotation_value(
    annotation_workbook: Path, row_index: int, header: str, value: str
) -> None:
...
def _add_extra_annotation_column(
    annotation_workbook: Path, header: str, value: str
) -> None:
    workbook = load_workbook(annotation_workbook)
    worksheet = workbook[ANNOTATION_SHEET]
...
def test_evaluate_uses_majority_reference_and_reports_artifacts(tmp_path: Path) -> None:
    ...
    replacement = next(
        label for label in ("positive", "negative", "mixed") if label != current_value
    )
    _set_annotation_value(annotator_b, 2, "Context Tone", replacement)
    _add_extra_annotation_column(
        annotator_b, "extra_reviewer_note", "not used in metrics"
    )
    ...
    _prepare_llm_artifact(
        mapping_workbook, study_config, llm_root, "exp-beta", run_id="run-02"
    )
    invalid_llm_dir = llm_root / "bad-exp"
    invalid_llm_dir.mkdir(parents=True, exist_ok=True)
    (invalid_llm_dir / "predictions.jsonl").write_text(
        '{"participant_code": "ANN_0001"}\n', encoding="utf-8"
    )
    ...
    report_text = (tmp_path / "evaluation_outputs" / "alignment_report.md").read_text(
        encoding="utf-8"
    )
    questionnaire_report_text = (
        tmp_path / "evaluation_outputs" / "alignment_report_questionnaire.md"
    ).read_text(encoding="utf-8")
    reference_df = pd.read_csv(
        tmp_path / "evaluation_outputs" / "human_reference_majority.csv"
    )
    family_metrics = pd.read_csv(
        tmp_path / "evaluation_outputs" / "alignment_family_metrics.csv"
    )
    human_pairwise = pd.read_csv(
        tmp_path / "evaluation_outputs" / "human_agreement_pairwise.csv"
    )
    ...
    human_manifest = json.loads(
        (tmp_path / "evaluation_outputs" / "human_artifacts_manifest.json").read_text(
            encoding="utf-8"
        )
    )

    context_vader = metrics[
        (metrics["comparison"] == "human_reference_vs_vader")
        & (metrics["field"] == "context_tone")
    ].iloc[0]
    ...
    assert {
        "questionnaire_vs_vader",
        "questionnaire_vs_llm:exp_alpha__run-01",
        "questionnaire_vs_llm:exp_beta__run-02",
    }.issubset(set(metrics["comparison"]))
    comparison_values = set(metrics["comparison"])
    assert "llm_vs_llm:exp_alpha__run-01__vs__exp_beta__run-02" in comparison_values
    assert "llm_vs_vader" in comparison_values
    questionnaire_vader_fields = set(
        metrics.loc[metrics["comparison"] == "questionnaire_vs_vader", "field"]
    )
    ...
    assert report_text.index("## Paper Figures (Lead)") < report_text.index(
        "## Hypothesis Verdict"
    )
    assert report_text.index("## Hypothesis Verdict") < report_text.index(
        "## General Results"
    )
    ...
    assert (
        "contradiction-focused qualitative analysis"
        in questionnaire_report_text.lower()
    )
    assert (
        "curated contradictory evidence examples" in questionnaire_report_text.lower()
    )
    assert "questionnaire_contradiction_overview.png" in questionnaire_report_text
    assert (
        "questionnaire_contradiction_unigram_wordcloud.png" in questionnaire_report_text
    )
    ...
    detailed_heatmaps_index = questionnaire_report_text.index(
        "## Item-Level Structure (Extended Figures)"
    )
    contradiction_index = questionnaire_report_text.index(
        "## Contradiction-Focused Qualitative Analysis"
    )
    ...
    assert (
        ...
        / "figures"
        / "alignment"
        / "human_nde_c_macro_f1_heatmap.png"
    ).exists()
    assert (
        tmp_path
        / "evaluation_outputs"
    ...
    assert (
        ...
        / "figures"
        / "alignment"
        / "questionnaire_extraction_item_scatter.png"
    ).exists()
    assert (
        tmp_path
        / "evaluation_outputs"
        / "figures"
        / "alignment"
        / "questionnaire_family_tradeoff_map.png"
    ).exists()
    assert (
        tmp_path
        / "evaluation_outputs"
        / "figures"
        / "alignment"
        / "questionnaire_tone_confusion_matrix.png"
    ).exists()
    assert (
        tmp_path
        / "evaluation_outputs"
    ...
    assert (
        tmp_path / "evaluation_outputs" / "questionnaire_contradictions.csv"
    ).exists()
...
def test_evaluate_can_skip_vader(tmp_path: Path) -> None:
    ...
    assert not any(
        str(value).startswith("vader_vs_llm:") for value in set(metrics["comparison"])
    )
...
def test_evaluate_can_export_pdf_figures(tmp_path: Path) -> None:
    ...
    assert (
        ...
        / "figures"
        / "alignment"
        / "questionnaire_family_tradeoff_map.pdf"
    ).exists()
    assert (
        tmp_path
        / "evaluation_outputs"
        / "figures"
        / "alignment"
        / "questionnaire_contradiction_overview.pdf"
    ).exists()


def test_evaluate_can_filter_to_selected_experiment_and_annotator(
    tmp_path: Path,
) -> None:
    study_config = FIXTURES / "study_test.toml"
    survey_csv = FIXTURES / "survey_fixture.csv"
    ...
    _prepare_llm_artifact(
        mapping_workbook, study_config, llm_root, "exp-one", run_id="run-01"
    )
    _prepare_llm_artifact(
        mapping_workbook, study_config, llm_root, "exp-two", run_id="run-02"
    )
...
def test_evaluate_can_filter_to_selected_prompt_variant_and_custom_output_dir(
    tmp_path: Path,
) -> None:
    study_config = FIXTURES / "study_test.toml"
    survey_csv = FIXTURES / "survey_fixture.csv"
    ...
    summary = json.loads(
        (custom_output_dir / "evaluation_summary.json").read_text(encoding="utf-8")
    )
    llm_manifest = json.loads(
        (custom_output_dir / "llm_artifacts_manifest.json").read_text(encoding="utf-8")
    )
